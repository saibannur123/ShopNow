...
userRouter.post("/change-password", async (req, res) => {
    ...
    try {
        ...
        if (!passCompare) {
            ...
            res.status(403).send({ message: "The provided password does not match the user" });
            return;
        }
        ...
        const result = await User.findOneAndUpdate({ email: user.email }, { password: newPassword });
