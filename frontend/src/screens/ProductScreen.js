    ...
    <div className="productScreen">
       { state.loader ? <div className="loader">Loading...</div> : 
            ...
            <Row > 
              ...
              <h5>Status: {state.product.inStock > 0 ? <span variant="success" className="badge bg-success">Available</span>:<span variant="danger" className="badge bg-danger">Out of Stock</span>}</h5> <hr></hr>
              {inCart(state.product) ? <button className="removeCart" onClick={removeFromCart}>Remove from Cart</button> : <button className="addCart" onClick={addToCart}>Add to Cart</button>}
              </Col>
            </Row>
          </Container>
        }
